...
def three_group_NE_maker(b, kappas, gamma=1 / 14, epsilon=0.0001):
    phi1, phi2, phi3 = 0.5, 0.5, 0
    k1, k2, k3 = kappas
    S1, S2, S3 = three_group_cvxpy(b, gamma, epsilon, phi1, phi2, phi3)
    p1 = p2 = p3 = 1
    print('S=', S1, S2, S3)
    X1 = b[0] / gamma * (S1 - phi1) + b[1] / gamma * (S2 - phi2) + b[2] / gamma * (S3 - phi3)
    X2 = b[3] / gamma * (S1 - phi1) + b[4] / gamma * (S2 - phi2) + b[5] / gamma * (S3 - phi3)
    X3 = b[6] / gamma * (S1 - phi1) + b[7] / gamma * (S2 - phi2) + b[8] / gamma * (S3 - phi3)
    print('X1=', X1)
    print('X2=', X2)
    print('X3=', X3)
    print((1 - epsilon) * np.exp(X1))
    print((1 - epsilon) * np.exp(X2))
    print((1 - epsilon) * np.exp(X3))
    print(X1 / (1 - epsilon))
    print(X2 / (1 - epsilon))
    print(X3 / (1 - epsilon))
    return
