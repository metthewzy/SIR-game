...
def derivative(X, Y):
    ...
    dXs = [X[i] - X[i - 1] for i in range(1, len(X))]
    dY_by_dX = [dY / dX for dY, dX in zip(dYs, dXs)]
    return dY_by_dX
...
def two_group_incentive():
    csv_name, paras = two_group_parameter_scenario(0)  # 0: low infection; 1: high infection
...
def plot_two_group_incentive(csv_name, paras):
    ...
    UG1s = [U1 * phi1 for U1, phi1 in zip(U1s, phi1s)]
    UG2s = [U2 * (1 - phi1) for U2, phi1 in zip(U2s, phi1s)]
    NE_idx = two_group_NE_finder(U1s, U2s)
    socials = [p1 * phi1 * Sbar1 + p2 * (1 - phi1) * Sbar2 for phi1, Sbar1, Sbar2 in zip(phi1s, Sbar1s, Sbar2s)]
    social_NE = socials[NE_idx]
    Delta1s, Delta2s, Bs = two_group_Delta_finder(csv_name, paras)

    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    ax1.plot(phi1s[:NE_idx], UG1s[:NE_idx], label='UG1')
    ax1.plot(phi1s[:NE_idx], UG2s[:NE_idx], label='UG2')
    ax1.set_xlabel(r'$\phi_1$')
    ax1.legend()
    plt.show()
    # new_p1s = [p1 + Delta1 for Delta1 in Delta1s]
    # new_p2s = [p2 + Delta2 for Delta2 in Delta2s]
    new_NEs = [max(p1 * Sbar1, p2 * Sbar2) for phi1, Sbar1, Sbar2 in zip(phi1s, Sbar1s, Sbar2s)]

    # metric1: Social (new NE) / Budget
    metric1 = [0 if B == 0 else NE / B for NE, B in zip(new_NEs, Bs)]
    # metric2: Social improvement / Budget
    metric2 = [0 if B == 0 else (NE - social_NE) for NE, B in zip(new_NEs, Bs)]

    fig = plt.figure()
    ax1 = fig.add_subplot(211)
    ax1.plot(Bs[:NE_idx], [NE for NE, B in zip(new_NEs[:NE_idx], Bs[:NE_idx])], label='metric 1')
    ax1.set_xlabel(r'Budget')
    ax1.legend()
    d_social_by_d_B = derivative(Bs[:NE_idx], new_NEs[:NE_idx])
    ax2 = fig.add_subplot(212)
    # ax2.plot(Bs[:NE_idx], [(NE) / B for NE, B in zip(new_NEs[:NE_idx], Bs[:NE_idx])], label='metric 2')
    ax2.plot(Bs[1:NE_idx], d_social_by_d_B, label='d Social / d Budget')
    ax2.set_xlabel(r'Budget')
    ax2.legend()
